				   ...
				   const enum display_orientation orientation)
{
	...
	/* set_lcd_margins() overwrites data->x/y_offset, so the current values are in
	 * whatever frame the CURRENT orientation uses. Normalise them back to the
	 * portrait frame first, otherwise margins are only correct for the specific
	 * transition order they were last computed in (e.g. cycling 270->180 applied
	 * the landscape x-margin to the 240-wide portrait axis: the write window
	 * overflowed the RAM width and every row wrapped -> diagonal shear). */
	bool cur_landscape = (data->orientation == DISPLAY_ORIENTATION_ROTATED_90 ||
			      data->orientation == DISPLAY_ORIENTATION_ROTATED_270);
	uint16_t base_x = cur_landscape ? data->y_offset : data->x_offset;
	uint16_t base_y = cur_landscape ? data->x_offset : data->y_offset;
